...
| `updateNote` | `{ id, title?, content?, shareType?, accessType?, expiryDate? }` | `ActionResult<Note>` — must verify `clerkUserId` ownership before writing, and must invalidate `note:{token}` in Redis (see `architecture.md` §5.2) |
