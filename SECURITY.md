...
| A single static JWT secret lets anyone who obtains it mint a session for _any_ note                 | Derive the signing key per-note (`HMAC(APP_SECRET, note.id + note.createdAt)`); short expiry (~5 min); httpOnly cookie instead of client-readable token.                                          |
...
- [x] Every mutation path invalidates the relevant Redis key in the same request as the DB write.
- [x] JWT secret is per-note derived, not a single static value.
- [x] Rate limits from `CodeStandards.md` are enforced at Edge Middleware, not just in application code (defense at the edge, before a serverless function even spins up).
- [x] Content rendering path has been manually checked for raw HTML injection.
- [x] `.env.example` contains no real secret values.
- [x] Webhook/callback endpoints verify signatures before doing any other work.
