...
import { toggleRevokeNote, deleteNote, regeneratePassword, getUserNotes } from '@/lib/actions/notes';
...
export const NotesTable: React.FC<NotesTableProps> = ({ initialNotes }) => {
  ...
  // Auto-refresh background polling (every 4s) so one-time consumptions & view counts update in real-time
  useEffect(() => {
  ...
  // Delete Note (Works for Revoked, Consumed, Expired, or Active notes)
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm border-collapse">
          <thead className="bg-slate-50 dark:bg-slate-800/50 text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-200 dark:border-slate-800">
            <tr>
              <th className="px-5 py-3.5 min-w-[160px]">Title</th>
              <th className="px-4 py-3.5 w-[130px] whitespace-nowrap">Share Type</th>
              <th className="px-4 py-3.5 w-[130px] whitespace-nowrap">Access Type</th>
              <th className="px-4 py-3.5 w-[170px] whitespace-nowrap">Expiry (IST)</th>
              <th className="px-4 py-3.5 w-[130px] whitespace-nowrap">Status</th>
              <th className="px-4 py-3.5 w-[110px] whitespace-nowrap">Total Views</th>
              <th className="px-5 py-3.5 w-[140px] text-right whitespace-nowrap">Actions</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
            {notes.map((note) => {
              ...
              const isConsumed = Boolean(note.consumedAt);
              ...
              return (
                ...
                >
                  <td className="px-5 py-4 font-semibold text-slate-900 dark:text-slate-100 max-w-[240px] truncate">
                    <Link href={`/notes/${note.id}`} className="hover:underline" title={note.title}>
                      {note.title}
                    </Link>
                  </td>
                  ...
                  <td className="px-4 py-4 text-slate-500 text-xs whitespace-nowrap" suppressHydrationWarning>
                    {istExpiry}
                  </td>
                  <td className="px-4 py-4 whitespace-nowrap">
                    {isConsumed ? (
                      <span className="inline-flex items-center gap-1 text-xs font-semibold text-purple-700 dark:text-purple-300 bg-purple-50 dark:bg-purple-950/40 border border-purple-200 dark:border-purple-900 px-2 py-0.5 rounded">
                        <ShieldAlert className="w-3 h-3" /> Consumed
                      </span>
                    ) : note.revoked ? (
                      <span className="inline-flex items-center gap-1 text-xs font-semibold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900 px-2 py-0.5 rounded">
                  ...
                  <td className="px-4 py-4 font-mono text-xs text-slate-700 dark:text-slate-300 whitespace-nowrap">
                  ...
                  <td className="px-5 py-4 text-right whitespace-nowrap">
                    <div className="inline-flex items-center justify-end gap-1">
                      ...
                      <button
                        ...
                        className="p-1.5 text-blue-600 hover:text-blue-800 dark:text-blue-400 rounded hover:bg-blue-50 dark:hover:bg-blue-950/40"
                      ...
                      <a
                        ...
                        className="p-1.5 text-slate-500 hover:text-slate-900 dark:hover:text-slate-100 rounded hover:bg-slate-100 dark:hover:bg-slate-800"
                      ...
                      <button
                        ...
                        title={note.revoked || isConsumed ? 'Restore / Unrevoke Note' : 'Revoke Note'}
                      >
                        {note.revoked || isConsumed ? <RotateCcw className="w-4 h-4" /> : <Ban className="w-4 h-4" />}
                      </button>

                      {/* Delete Note Button (Works for active, consumed, and revoked notes) */}
                      <button
                        onClick={() => setConfirmDeleteId(note.id)}
                        className="p-1.5 text-red-500 hover:text-red-700 dark:hover:text-red-400 rounded hover:bg-red-50 dark:hover:bg-red-950/40"
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 max-w-md w-full shadow-2xl relative space-y-4">
            ...
            <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl text-xs">
              ...
              <span className="font-bold flex items-center gap-1">
                {shareModalNote.consumedAt ? (
                  <span className="text-purple-600 flex items-center gap-1">
                    <ShieldAlert className="w-3.5 h-3.5" /> Consumed
                  </span>
            ...
            <div className="pt-2 flex justify-between items-center border-t border-slate-100 dark:border-slate-800">
              <button
                ...
                className={`text-xs font-semibold flex items-center gap-1 px-3 py-1.5 rounded-lg border ${
                  shareModalNote.revoked || shareModalNote.consumedAt
              ...
              >
                {shareModalNote.revoked || shareModalNote.consumedAt ? (
                  <>
                    <RotateCcw className="w-3.5 h-3.5" /> Restore Link
                  </>
                ) : (
                  <>
